...
from PyQt6.QtWidgets import (
    QWidget, QLabel, QListWidget, QHBoxLayout, QVBoxLayout,
...
from PyQt6.QtCore import Qt, pyqtProperty, QPropertyAnimation, QEasingCurve, pyqtSignal, QTimer, QRectF
...
# 登录/注册页面(独立模块)
from loginPage import LoginPage, EnrollPage
...
class UserAvatar(QFrame):
    ...
    用户头像(圆形)
    ==============
    未登录: 显示默认头像 👤 , 悬停提示"未登录", 点击→登录页
    已登录: 显示用户头像(有设置则加载, 否则默认), 悬停显示用户信息, 点击→账户页
    """
    ...
    def refresh(self):
        """
        刷新头像显示(登录/退出后调用)
        根据 currentUser 的登录状态决定显示内容
        """
        from currentUser import is_logged_in, get_current_user, get_avatar_path

        if is_logged_in():
            user = get_current_user()
            avatarPath = get_avatar_path()
            # 尝试加载用户头像(有实际图片则显示, 否则用默认 👤)
            if avatarPath and os.path.exists(avatarPath) and not avatarPath.endswith(".png"):
                # 如果能加载到图片, 显示图片
    ...
    def _user_info_html(self) -> str:
        """构造用户信息富文本(悬停显示)"""
        from currentUser import is_logged_in, get_current_user
        if is_logged_in():
            user = get_current_user()
            return (f"<b>{get_text('user.tooltip_title')}</b><br/>"
                    f"{user['username']}<br/>"
                    f"{get_text('user.nickname')}: {user.get('nickname', '')}<br/>"
                    f"Email: {user.get('email', '')}")
        else:
            return f"<b>{get_text('user.tooltip_title')}</b><br/>{get_text('user.not_logged_in')}<br/>{get_text('user.click_to_login')}"
...
class MainWindow(GUI):
    ...
    def __init__(self, uiRsp):
        ...
        # 登录/注册页(不加入左侧导航)
        self.add_page(LoginPage(self), "login")
        self.add_page(EnrollPage(self), "enroll")
        ...
        # 导航联动 + 默认进入安全概览(设计图主页)
        # 记录当前合法导航行(初始为概览行 0), 用于未登录拦截时回退导航高亮
        self._last_nav_row = 0
        self._nav.currentRowChanged.connect(self._on_nav_changed)
        self.switch_page("overview")

        # 启动后延迟尝试自动登录(等事件循环与网络就绪后, 读取本地 cookie 免密登录)
        QTimer.singleShot(500, self.__tryAutoLogin)

        # F11 切换全屏
        QShortcut(QKeySequence("F11"), self, activated=self.toggle_fullscreen)

    def __tryAutoLogin(self):
        """
        软件启动时尝试自动登录
        ======================
        读取本地 cookie(存在则发起自动登录):
          - 成功 → 写入 currentUser 并刷新概览/账户页登录状态
          - 失败(无 cookie / ip 或设备码不匹配 / 到期) → 静默忽略,用户可手动登录
        """
        from authService import autoLogin, hasStoredCookie

        # 无本地 cookie 则直接返回,保持未登录状态
        if not hasStoredCookie():
            return
        ...
        if result["code"] == 200:
            ...
            # 刷新概览页与账户页的登录状态显示
            overviewPage = self.get_page("overview")
            if overviewPage and hasattr(overviewPage, "refreshLoginState"):
                overviewPage.refreshLoginState()
            accountPage = self.get_page("account")
            if accountPage and hasattr(accountPage, "refreshLoginState"):
                accountPage.refreshLoginState()
    ...
    def switch_page(self, page_id, animate=False):
        """
        切换页面; 账户页/全屏画面页/登录页/注册页为沉浸式: 隐藏标题栏与左侧导航, 内容铺满窗口
        切回概览页时自动刷新登录状态
        ...
        is_immersive = page_id in ("account", "live", "login", "enroll")
    ...
    def _on_nav_changed(self, row):
        """导航行变化 → 切换页面; 功能页需登录, 未登录弹窗拦截并回退导航高亮"""
        if 0 <= row < len(NAV_ITEMS):
            pageId = NAV_ITEMS[row][1]
            # 概览页为登录入口无需拦截; 其余功能页需登录, 未登录弹窗并回退导航高亮
            if pageId != "overview" and not self._requireLogin(pageId):
                self._nav.blockSignals(True)
                self._nav.setCurrentRow(self._last_nav_row)
                self._nav.blockSignals(False)
                return
            # 放行: 记录当前合法导航行, 切换页面
            self._last_nav_row = row
            self.switch_page(pageId)

    def _requireLogin(self, pageId):
        """
        校验目标功能页是否允许访问(需登录)
        未登录 → 弹窗提示"您还未登录，请登录后再使用"并返回 False(拦截)
        已登录 → 返回 True(放行)
        :param pageId: 目标页面 id<str>
        :return: 是否允许进入<bool>
        """
        from currentUser import is_logged_in
        if is_logged_in():
            return True
        # 未登录 → 弹窗提示, 阻止进入功能页
        QMessageBox.warning(
            self,
            get_text("login.need_login_title"),
            get_text("login.need_login"),
        )
        return False
...
class OverviewPage(QWidget):
    ...
    def __init__(self, gui):
        ...
        # ── 右上角用户区域: 未登录→登录按钮, 已登录→头像+用户名 ──
        userArea = QVBoxLayout()
        userArea.setSpacing(4)

        # 登录按钮(未登录时显示)
        self.loginBtn = QPushButton(get_text("user.btn_login"))
        self.loginBtn.setObjectName("loginNavBtn")
        self.loginBtn.clicked.connect(lambda: self._gui.switch_page("login"))

        # 用户名标签(登录后显示)
        self.userNameLabel = QLabel("")
        self.userNameLabel.setObjectName("userInfoLabel")
        self.userNameLabel.setAlignment(Qt.AlignmentFlag.AlignRight)

        userArea.addWidget(self.loginBtn)
        userArea.addWidget(self.userNameLabel)
        head.addLayout(userArea)

        # ── 右上角用户头像(悬停显示用户信息, 点击进入账户页/登录页) ──
        self.avatar = UserAvatar()
        self.avatar.clicked.connect(self.__onAvatarClick)
    ...
    def refreshLoginState(self):
        ...
        if is_logged_in():
            user = get_current_user()
            self.loginBtn.hide()
            self.userNameLabel.setText(user.get("username", ""))
...
class AccountPage(QWidget):
    ...
    账户管理页
    ==========
    未登录: 显示"未登录"提示 + 登录/注册按钮
    已登录: 显示大头像 + 用户名/昵称/邮箱/角色 + 退出登录按钮
    """
    ...
    def __init__(self, gui):
        ...
        layout.setContentsMargins(0, 0, 0, 0)
        layout.addLayout(head)
        layout.addWidget(self.centerWidget, 1)
    ...
    def refreshLoginState(self):
        ...
        while self.centerLayout.count():
            item = self.centerLayout.takeAt(0)
            if item.widget():
        ...
        if is_logged_in():
            user = get_current_user()
            self.__buildLoggedInUI(user)
        else:
            self.__buildNotLoggedInUI()

        self.centerLayout.addStretch(3)

    def __buildLoggedInUI(self, user):
        """构建已登录 UI"""
        # 大头像
        bigAvatar = QLabel("👤")
        bigAvatar.setObjectName("bigAvatar")
        bigAvatar.setAlignment(Qt.AlignmentFlag.AlignCenter)
        bigAvatar.setFixedSize(96, 96)

        avatarRow = QHBoxLayout()
        avatarRow.addStretch(1)
        avatarRow.addWidget(bigAvatar)
        avatarRow.addStretch(1)
        self.centerLayout.addLayout(avatarRow)
        self.centerLayout.addSpacing(8)

        # 用户名
        userName = QLabel(user.get("username", ""))
        userName.setObjectName("cardTitle")
        userName.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.centerLayout.addWidget(userName)

        # 用户信息行
        infoItems = [
            (get_text("user.nickname"), user.get("nickname", "-")),
            (get_text("user.email"), user.get("email", "-")),
            (get_text("user.role"), "管理员" if user.get("role") == "admin" else "普通用户"),
        ]
        for key, value in infoItems:
            infoRow = QHBoxLayout()
            infoRow.addStretch(1)
            keyLabel = QLabel(f"{key}:")
            keyLabel.setObjectName("accountInfoKey")
            infoRow.addWidget(keyLabel)
            infoRow.addSpacing(8)
            valLabel = QLabel(value)
            valLabel.setObjectName("accountInfoValue")
            infoRow.addWidget(valLabel)
            infoRow.addStretch(1)
            self.centerLayout.addLayout(infoRow)

        self.centerLayout.addSpacing(16)

        # 退出登录按钮
        logoutRow = QHBoxLayout()
        logoutRow.addStretch(1)
        logoutBtn = QPushButton(get_text("user.btn_logout"))
        logoutBtn.setObjectName("logoutBtn")
        logoutBtn.clicked.connect(self.__onLogout)
        logoutRow.addWidget(logoutBtn)
        logoutRow.addStretch(1)
        self.centerLayout.addLayout(logoutRow)

    def __buildNotLoggedInUI(self):
        """构建未登录 UI"""
        hint = QLabel(get_text("user.not_logged_in"))
        hint.setObjectName("placeholderTitle")
        hint.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.centerLayout.addWidget(hint)

        self.centerLayout.addSpacing(12)

        loginRow = QHBoxLayout()
        loginRow.addStretch(1)
        loginBtn = QPushButton(get_text("user.btn_login"))
        loginBtn.setObjectName("primaryBtn")
        loginBtn.clicked.connect(lambda: self._gui.switch_page("login"))
        loginRow.addWidget(loginBtn)
        loginRow.addStretch(1)
        self.centerLayout.addLayout(loginRow)

    def __onLogout(self):
        """退出登录(调用 authService.logout() + 清除本地状态)"""
        from currentUser import logout

        # 如果有网络连接, 调用服务端登出
        try:
            from authService import logout as authLogout, isConnected
            if isConnected():
                authLogout()
        except Exception:
            pass
        ...
        self.refreshLoginState()
        # 刷新概览页(如果存在)
        overviewPage = self._gui.get_page("overview")
        if overviewPage and hasattr(overviewPage, "refreshLoginState"):
            overviewPage.refreshLoginState()
