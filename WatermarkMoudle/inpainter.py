...
def hasLamaModel() -> bool:
    """LaMa 模型文件是否就绪"""
    return os.path.exists(LAMA_MODEL_PATH)
...
class Inpainter:
    ...
    def _loadLama(self, use_gpu):
        ...
        if not hasLamaModel():
            print("[inpainter] ⚠ LaMa 模型不存在, 降级为 fast 模式(OpenCV inpaint)。")
            print(f"[inpainter]   请放置模型文件: {LAMA_MODEL_PATH}")
            self._mode = "fast"
            return
        try:
            ...
            print(f"[inpainter] LaMa 模型加载成功({'GPU' if gpu_used else 'CPU'})")
        except Exception as e:
            print(f"[inpainter] LaMa 加载失败({e}), 降级为 fast 模式")
            self._mode = "fast"
    ...
    def inpaint(self, frame, mask):
        ...
        if self._mode == "lama" and self._session is not None:
            try:
                ...
                print(f"[inpainter] LaMa 推理失败({e}), 本帧回退 fast")
