...
def main():
    ...
    freezeConfig.set("cooldown_seconds", 2)
    freezeConfig.set("cpu_threshold", 10.0)
    # 本机 C 盘余量可能不足(真实 disk_full 会抢在 cpu_high 前报警,
    # 冷却期会压掉本测试目标), 临时调低磁盘阈值屏蔽干扰
    freezeConfig.set("disk_free_threshold", 0.1)
    try:
        ...
        # 无论成败都恢复默认配置(不依赖先前状态, 避免残留影响后续测试)
