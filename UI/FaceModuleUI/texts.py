...
TEXTS = {
    LANG_ZH: {
        ...
        "watermark.input.placeholder": "选择要去水印的视频文件",
        "watermark.output": "输出文件夹",
        "watermark.output.placeholder": "选择输出文件夹(留空=输入同目录)",
        "watermark.btn.browse": "浏览",
        "watermark.mode": "水印类型",
        "watermark.mode.static": "静止水印(台标/角标)",
        "watermark.mode.dynamic": "动态水印(滚动/移动)",
        "watermark.quality": "修复质量",
        "watermark.quality.fast": "快速(OpenCV)",
        "watermark.quality.lama": "高清(LaMa)",
        "watermark.quality.fast.hint": "适用: 小面积水印/台标/简单文字, 背景简单的视频。速度: 极快(约 5-20ms/帧, 1080p 一小时视频约 1-3 分钟)。局限: 大面积或复杂纹理背景可能留下淡化痕迹。",
        "watermark.quality.lama.hint": "适用: 大面积水印、复杂纹理背景、追求无痕效果。速度: GPU 约 100-500ms/帧, CPU 约 2s/帧(1080p 一小时视频 GPU 约 10-30 分钟)。局限: 半透明水印过淡时可能漏掉部分笔画。",
        "watermark.gpu": "GPU 加速",
        "watermark.gpu.auto": "自动(可用则用)",
        "watermark.gpu.on": "开启",
        "watermark.gpu.off": "关闭",
        "watermark.bbox": "手动水印区域(可选)",
        "watermark.bbox.placeholder": "留空=自动检测; 格式 x1,y1,x2,y2",
        "watermark.bbox.select": "框选...",
        "watermark.select.title": "手动框选水印区域",
        "watermark.select.hint": "在画面上按住鼠标左键拖拽框选水印位置(支持多选); 可播放视频或拖动进度条定位到水印最清晰的一帧; 已选区域以绿色框+编号显示。",
        "watermark.select.play": "播放",
        "watermark.select.pause": "暂停",
        "watermark.select.undo": "撤销",
        "watermark.select.clear": "清空",
        "watermark.select.confirm": "确认",
        "watermark.select.cancel": "取消",
        "watermark.select.frame": "第 {0} / {1} 帧",
        "watermark.select.rects": "已选 {0} 个区域: {1}",
        "watermark.select.empty_rects": "尚未框选任何区域(可框选多个水印)",
        "watermark.status.select_done": "已框选 {0} 个水印区域, 可直接开始处理",
        "watermark.eta": "预计剩余 {0}",
        "watermark.open": "打开保存位置",
        "watermark.sound": "完成提示音",
        "watermark.log.title": "处理日志",
        "watermark.log.empty": "处理过程与最终结果将显示在这里",
        "watermark.log.need_input": "请先选择输入视频",
        "watermark.log.region_error": "水印区域格式应为 x1,y1,x2,y2(像素), 多个用分号分隔",
        "watermark.log.start": "任务开始: {0}",
        "watermark.log.params": "参数: 引擎={0} | GPU={1} | 定位={2} | 模式={3}",
        "watermark.log.output": "输出: {0}",
        "watermark.log.done": "处理完成 | 总耗时 {0} | 平均 {1}ms/帧 | 帧数 {2} | 引擎 {3} | 水印区域 {4} | 说明 {5} | 输出 {6}",
        "watermark.log.fail": "处理失败: {0}",
        "watermark.log.cancelled": "任务已取消",
        "watermark.log.hint_fast": "提示: 快速引擎对大面积/复杂背景可能留痕, 如不满意可改用高清(LaMa)引擎重试",
        "watermark.log.hint_residue": "提示: 若水印仍有残留, 可框选更大范围重试(系统会自动收紧到水印像素)",
        "watermark.log.stage.start": "启动",
        "watermark.log.stage.sample": "采样",
        "watermark.log.stage.analyze": "分析",
        "watermark.log.stage.refine": "细化",
        "watermark.log.stage.engine": "引擎",
        "watermark.log.stage.process": "处理",
        "watermark.log.stage.done": "完成",
        "watermark.log.stage.fail": "失败",
        "watermark.log.stage.info": "信息",
    ...
    LANG_EN: {
        ...
        "watermark.input.placeholder": "Select the video to remove watermark",
        "watermark.output": "Output Folder",
        "watermark.output.placeholder": "Select an output folder (empty = same dir)",
        ...
        "watermark.quality.lama": "HD (LaMa AI)",
        "watermark.quality.fast.hint": "For: small watermarks/logos/simple text on simple backgrounds. Speed: very fast (~5-20ms/frame; 1h 1080p video ~1-3 min). Limit: may leave faded traces on large or complex-texture areas.",
        "watermark.quality.lama.hint": "For: large watermarks, complex-texture backgrounds, trace-free results. Speed: GPU ~100-500ms/frame, CPU ~2s/frame (1h 1080p: 10-30 min on GPU). Limit: very faint semi-transparent watermarks may be partially missed.",
        "watermark.gpu": "GPU Acceleration",
        "watermark.gpu.auto": "Auto (use when available)",
        "watermark.gpu.on": "On",
        "watermark.gpu.off": "Off",
        "watermark.bbox": "Manual Region (optional)",
        "watermark.bbox.placeholder": "Empty = auto-detect; format x1,y1,x2,y2",
        "watermark.bbox.select": "Select...",
        "watermark.select.title": "Select Watermark Regions",
        "watermark.select.hint": "Drag with the left mouse button to box the watermark(s) (multiple allowed); play the video or drag the timeline to find the clearest frame; selected regions show as green boxes with numbers.",
        "watermark.select.play": "Play",
        "watermark.select.pause": "Pause",
        "watermark.select.undo": "Undo",
        "watermark.select.clear": "Clear",
        "watermark.select.confirm": "Confirm",
        "watermark.select.cancel": "Cancel",
        "watermark.select.frame": "Frame {0} / {1}",
        "watermark.select.rects": "{0} region(s) selected: {1}",
        "watermark.select.empty_rects": "No region selected yet (multiple watermarks allowed)",
        "watermark.status.select_done": "{0} watermark region(s) selected, ready to process",
        "watermark.eta": "ETA {0}",
        "watermark.open": "Open Output Folder",
        "watermark.sound": "Completion Sound",
        "watermark.log.title": "Processing Log",
        "watermark.log.empty": "Processing progress and final result will show here",
        "watermark.log.need_input": "Please select an input video first",
        "watermark.log.region_error": "Region format should be x1,y1,x2,y2 (px), multiple separated by ';'",
        "watermark.log.start": "Task started: {0}",
        "watermark.log.params": "Params: engine={0} | GPU={1} | locate={2} | mode={3}",
        "watermark.log.output": "Output: {0}",
        "watermark.log.done": "Done | total {0} | avg {1}ms/frame | frames {2} | engine {3} | region {4} | note {5} | output {6}",
        "watermark.log.fail": "Failed: {0}",
        "watermark.log.cancelled": "Task cancelled",
        "watermark.log.hint_fast": "Tip: fast engine may leave traces on large/complex areas; retry with HD (LaMa) engine if not satisfied",
        "watermark.log.hint_residue": "Tip: if the watermark still remains, box a larger area and retry (auto-tightened to watermark pixels)",
        "watermark.log.stage.start": "START",
        "watermark.log.stage.sample": "SAMPLE",
        "watermark.log.stage.analyze": "ANALYZE",
        "watermark.log.stage.refine": "REFINE",
        "watermark.log.stage.engine": "ENGINE",
        "watermark.log.stage.process": "PROCESS",
        "watermark.log.stage.done": "DONE",
        "watermark.log.stage.fail": "FAIL",
        "watermark.log.stage.info": "INFO",
